...
# -------------------------------------------------------------------------
# 1.  Adjusted-inflation metrics  (blue/orange/yellow × pre/post/total)
# -------------------------------------------------------------------------
color_map <- c("b" = "blue", "o" = "orange", "y" = "yellow")

infl_summary <- df_with_preds %>%
  filter(popped != 1) %>%                                # only trials that did NOT pop
  mutate(
  ...
  pivot_wider(names_from = c(color, period),
              ...
              names_glue  = "{color}_{period}")           # blue_pre, orange_post, …
...
total_summary <- df_with_preds %>%
  ...
  pivot_wider(names_from = color,
              values_from = mean_infl,
              names_glue  = "{color}_total")              # blue_total …

# -------------------------------------------------------------------------
# 2.  Reaction-time metric
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# 4.  Gather all new participant-level columns
#     (df_params_combined already has fourpar_ / ewmv_ / stl_ params)
# -------------------------------------------------------------------------
participant_enhanced <- df_params_combined %>%             # <- already unique rows
...
# -------------------------------------------------------------------------
# 5.  Attach new columns back to every trial row
# -------------------------------------------------------------------------
df_with_preds <- df_with_preds %>%
  left_join(participant_enhanced %>%
              select(participant_id,
                     starts_with("blue_"),
                     starts_with("orange_"),
                     starts_with("yellow_"),
                     average_rt_total,
                     schizotypy_aggregate),
            by = "participant_id")

# -------------------------------------------------------------------------
# 6.  Write full trial-level data to CSV
# -------------------------------------------------------------------------
write_csv(df_with_preds, "bart_rl_180_modeled.csv")

# -------------------------------------------------------------------------
# 7.  Build the requested summary_dataframe
# -------------------------------------------------------------------------
summary_cols <- c(
  "participant_id", "total_pops", "task_time",
  # adjusted-inflation columns
  "blue_pre","orange_pre","yellow_pre",
  "blue_post","orange_post","yellow_post",
  "blue_total","orange_total","yellow_total",
  # reaction time + aggregate scores
...
summary_dataframe <- df_with_preds %>%
