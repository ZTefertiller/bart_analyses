...
run_stan_fit <- function(data, fit_name = "default") {
  ...
  memory_used <- mem_change({
    mod <- stan_model(file
        ="/Users/zachtefertiller/Desktop/bart_analyses/modeling/emwv_test.stan")
    fit <- sampling(
      mod,
      data = data,          # stan data
...
library(cmdstanr)
library(pryr)  # for mem_change

cmd_stan_fit <- function(data, fit_name = "default") {
  model_path <- "/Users/zachtefertiller/Desktop/bart_analyses/modeling/ewmv_vec.stan"
  output_path <- paste0("/Users/zachtefertiller/Desktop/", fit_name, ".rds")
...
```{r}
cmd_stan_fit(stan_data, "ewmv")
```

```{r}
ewmv_test <- cmd_stan_fit(stan_data, "ewmv_test")
```
...
```{r load fit}
fit <- readRDS("/Users/zachtefertiller/Desktop/beta_split.rds")
```
...
log_lik_array <- extract(fit, pars = "log_lik")$log_lik  # now dim = (iterations, nsub, ntrial)

dim(log_lik_array) <- c(dim(log_lik_array)[1], dim(log_lik_array)[2] * dim(log_lik_array)[3])
