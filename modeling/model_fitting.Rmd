...
# -------------------------------------------------------------------------
# 3) schizotypy aggregate ---------------------------------------------------
schizo_summary <- df_with_preds %>%
  group_by(participant_id) %>%
  summarise(
...
# -------------------------------------------------------------------------
# 4) gather participant-level info (model params + new metrics) -------------
param_cols <- c(
...
participant_enhanced <- df_params_combined %>%          # already one row per id
  left_join(infl_summary,  by = "participant_id") %>%
  left_join(total_summary, by = "participant_id") %>%
  left_join(rt_summary,    by = "participant_id") %>%
  left_join(schizo_summary,by = "participant_id")

# -------------------------------------------------------------------------
# 5) add those columns onto every trial row ---------------------------------
df_with_preds <- df_with_preds %>%
  left_join(
...
# -------------------------------------------------------------------------
# 6) write trial-level CSV ---------------------------------------------------
write_csv(df_with_preds, "bart_rl_modeled.csv")

# -------------------------------------------------------------------------
# 7) build summary_dataframe -------------------------------------------------
summary_cols <- c(
...
write_csv(summary_dataframe, "bart_rl_summary.csv")
