...
param_extract <- function(fit, sub_ids,
  ...
  # 1) get the summary matrix directly from the stanfit:
  sum_mat <- summary(fit, pars = pars)$summary
  # sum_mat is a matrix with rows like "vwin[1]", "vwin[2]", ... 
  # and columns including "mean" and "Rhat"
  
  # 2) helper to pull out mean & Rhat for one parameter
  ...
  # 4) build the results data.frame
  df <- data.frame(participant_id = sub_ids, stringsAsFactors = FALSE)
  for (p in pars) {
    df[[paste0(p, "_mean")]] <- stats_list[[p]]$mean
