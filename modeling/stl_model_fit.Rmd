...
run_stan_fit <- function(data, fit_name = "default") {
  #uncomment below if you want csv files for each chain
...
```{r}
blue_stl_all <- run_stan_fit(stan_data_blue)
```
...
```{r make array for estimated parameters}
param_extract <- function(fit_summary) {
  fit_summary <- fit_summary$summary
  vwin <- fit_summary[grep("^vwin", rownames(fit_summary)), "mean"]
  vloss <- fit_summary[grep("^vloss", rownames(fit_summary)), "mean"]
  omegaone <- fit_summary[grep("^omegaone", rownames(fit_summary)), "mean"]
  beta <- fit_summary[grep("^beta", rownames(fit_summary)), "mean"]
  
  params <- data.frame(
    participant = 1:length(vwin),
    vwin,
    vloss,
    omegaone,
    beta,
...
param_extract(blue_stl_summary)
